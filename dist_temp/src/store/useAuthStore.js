...
exports.useAuthStore = exports.getRandomPremiumAvatar = exports.extractFirstName = void 0;
const zustand_1 = require("zustand");
const react_native_1 = require("react-native");
const FileSystem = __importStar(require("expo-file-system"));
const AUTH_FILE = FileSystem['documentDirectory'] + 'mockmaxxing_auth_v2.json';
...
const generateFantasyName = (email) => {
...
const extractFirstName = (fullName) => {
    ...
    if (first.toLowerCase() === 'google')
        return 'Google';
    if (first.toLowerCase() === 'apple')
        return 'Apple';
...
exports.useAuthStore = (0, zustand_1.create)((set, get) => {
    ...
    return {
            ...
            try {
                ...
                if (storedStr) {
                    ...
                    if (parsed.user) {
                        ...
                        if (restoredUser.email === 'brad@mockmax.com' ||
                ...
                if (restoredUser && !restoredUser.firstName) {
                    restoredUser.firstName = (0, exports.extractFirstName)(restoredUser.name || restoredUser.email || 'COACH');
                }
                Object.keys(restoredRegistered).forEach(email => {
                    if (!restoredRegistered[email].firstName) {
                        restoredRegistered[email].firstName = (0, exports.extractFirstName)(restoredRegistered[email].name || email);
                    }
                });
                set({
                    user: restoredUser,
        ...
        loginWithProvider: async (provider, preferences, customName) => {
            ...
            const firstName = (0, exports.extractFirstName)(name);
            ...
            if (existingRegisteredUser) {
                ...
                finalFirstName = existingRegisteredUser.firstName || (0, exports.extractFirstName)(finalName);
            }
            else {
        ...
        registerWithEmail: async (email, passwordHash, name, firstName, preferences, phoneNumber) => {
            const { registeredUsers } = get();
            const normalizedEmail = email.trim().toLowerCase();
            if (registeredUsers[normalizedEmail]) {
                return false; // Email already exists
            }
            const avatarUrl = (0, exports.getRandomPremiumAvatar)();
            const finalName = name.trim() || `@${firstName}_${Math.floor(100 + Math.random() * 900)}`;
            const newRegisteredUser = {
                email: normalizedEmail,
                passwordHash,
                name: finalName,
                firstName: firstName || (0, exports.extractFirstName)(finalName),
                avatarUrl,
                preferences,
        ...
        loginWithEmail: async (email, passwordHash) => {
            const { registeredUsers } = get();
            const normalizedEmail = email.trim().toLowerCase();
            ...
            const newUser = {
                ...
                firstName: existingUser.firstName || (0, exports.extractFirstName)(existingUser.name || 'COACH'),
        ...
        updateProfile: async (name, email, phoneNumber) => {
            const { user, registeredUsers } = get();
            if (!user)
            ...
            const updatedUser = {
                ...
                name: finalName,
                email: trimmedEmail,
                firstName: (0, exports.extractFirstName)(finalName),
        ...
        resetPasswordWithPhone: async (email, phoneNumber, newPasswordHash) => {
            const { registeredUsers } = get();
            const normalizedEmail = email.trim().toLowerCase();
            ...
            const newUser = {
                ...
                firstName: existingRecord.firstName || (0, exports.extractFirstName)(existingRecord.name || 'COACH'),
